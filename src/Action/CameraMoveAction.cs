...
namespace TaleUtil
{
    public class CameraMoveAction : TaleUtil.Action
    {
        private enum State
        {
            SETUP,
        ...
        private CameraMoveAction() { }

        public CameraMoveAction(Vector2 pos, float transitionDuration, TaleUtil.Delegates.InterpolationDelegate interpolation)
        {
            TaleUtil.Assert.NotNull(TaleUtil.Props.camera, "CameraMoveAction requires a main camera object (which could not be found)");
        ...
        {
            CameraMoveAction clone = new CameraMoveAction();
            clone.pos = pos;
