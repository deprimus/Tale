...
namespace TaleUtil {
    public class CinematicBackgroundAction : Action {
        ...
        public override string ToString() {
            return string.Format("CinematicBackgroundAction (<color=#{0}>{1}</color>)", ColorUtility.ToHtmlStringRGBA(master.Config.Debug.INFO_ACCENT_COLOR_PRIMARY), state.ToString());
        }
    }
}